...
(*
Lemma lambda_is_eigenvalue (n:nat) (A: 'M[complex R]_n.+1): 
...
Proof.
intros.
assert (lambda A i = Jordan_form A i i).
{ unfold Jordan_form. admit. }
rewrite H. rewrite eigenvalue_root_char.
apply eigen_diag.
*)
(*
...
Theorem iter_convergence: 
  ...
  A \in unitmx ->
   ...
   (let S_mat:= RtoC_mat (oppmx (mulmx ((invmx A1)) A2)) in 
     (forall (i: 'I_n.+1), 
      @eigenvalue (complex_fieldType _) n.+1 S_mat (lambda S_mat i) ->
...
{ intros. simpl.
  ...
  rewrite H4. rewrite !mul1mx. rewrite -Mopp_mult_r.
  ...
  + rewrite expr1. rewrite -[LHS]mul1mx Mopp_mult_r.
    ...
    { rewrite !mulmxDr. 
      ...
      { rewrite mulmxA. rewrite Mopp_mult_l. rewrite mulmxA.
        rewrite -Mopp_mult_r. 
        assert (A1 *m invmx A1 = 1%:M). { by apply inverse_A. }
        rewrite H6 //=. clear H6. rewrite -mulmxA.
        by rewrite -!Mopp_mult_l mul1mx. 
      } rewrite H6.
      rewrite mulmxA. 
      assert (A1 *m invmx A1 = 1%:M). { by apply inverse_A. }
      rewrite H7. rewrite mul1mx. 
      rewrite -H0. 
      ...
      { rewrite -!Mopp_mult_r. rewrite Mopp_mult_l -mulmxDl.
        ...
        rewrite H8. by rewrite addr0.
      } rewrite -addrA. by rewrite H8.
    } by rewrite H6.
  + rewrite exprS. specialize (H4 x0 (m.+1)).
    rewrite -mulmxA -IHm.
    ...
    { assert (oppmx (mulmx (invmx A1) A2) = mulmx (invmx A1) (oppmx A2)).
      ...
      rewrite H12. 
      assert (mulmx (invmx A1) A1 = 1%:M). {  by apply inverse_A. } rewrite -H13.
      assert (mulmx (mulmx (invmx A1) A1) (X_m m.+2 x0 b A1 A2) = 
                mulmx (invmx A1) (mulmx A1 (X_m m.+2 x0 b A1 A2))).
      { symmetry. apply mulmxA. } rewrite H14.
...
{ split.
  + intros.
    ...
    - intros. specialize (H5 x0 n0).
      rewrite -H5. rewrite -mat_vec_unfold. rewrite vec_norm_R_C.
      simpl. by rewrite /v.
    - apply H6.
  + intros. 
    assert ((x0 - X) = 0 \/ (x0 - X) != 0).
    { assert (RtoC_vec (x0 - X) = 0 \/ RtoC_vec (x0 - X) != 0).
      { apply vec_is_zero_or_not. } destruct H7.
      + left.
        apply matrixP. unfold eqrel. intros. rewrite [RHS]mxE.
        apply matrixP in H7. unfold eqrel in H7. 
        specialize (H7 x y). rewrite mxE in H7. 
        assert (((x0 - X) x 0 +i* 0)%C == (0: 'M[complex R]_(n.+1,1)) x y).
        { by apply /eqP. } rewrite eq_complex in H8.
        simpl in H8.
        assert (((x0 - X) x 0 == Re ((0: 'M[complex R]_(n.+1,1)) x y)) /\
                   (0 == Im ((0: 'M[complex R]_(n.+1,1)) x y))).
        { by apply /andP. } destruct H9. 
        assert ((x0 - X) x 0 = Re ((0: 'M[complex R]_(n.+1,1)) x y)). 
        { by apply /eqP. } 
        assert (y = 0). { by apply ord1. } rewrite H12. rewrite H11.
        by rewrite mxE.
      + right. apply /cV0Pn.
        assert (exists i, RtoC_vec (x0 - X) i 0 != 0).
        { by apply /cV0Pn. } destruct H8 as [i H8]. exists i.
        rewrite eq_complex in H8. 
        assert (~~ (Re (RtoC_vec (x0 - X) i 0) == Re 0) \/
                ~~(Im (RtoC_vec (x0 - X) i 0) == Im 0)).
        { by apply /nandP. } destruct H9.
        - rewrite mxE //= in H9.
        - rewrite mxE //= in H9. 
          assert ( 0%Re <> 0%Re). { by apply /eqP. } by contradict H10.
      } destruct H7.
      ...
      * apply (is_lim_seq_ext (fun _ => 0) 
        ...
        ++ intros. rewrite //=. 
           ...
           { apply matrixP. unfold eqrel. intros. rewrite [RHS]mxE.
             ...
             { by apply /forallP. }
             specialize (H9 x).
             assert (forall j, (x0 - X) x j == 0).
             { by apply /forallP. } specialize (H10 y).
             apply /eqP. apply H10.
           } rewrite H8. rewrite mulmx0.
    ...
    - apply (is_lim_seq_ext (fun m:nat => vec_norm (mulmx ((oppmx 
      ...
      * apply (is_lim_seq_ext (fun m : nat =>
        ...
        ++ intros. split.
          ...
          { apply mat_vec_unfold. } rewrite H8.
          apply /RleP. apply matrix_norm_compat.
          rewrite //=. 
          assert ((exists i: 'I_n.+1, addmx x0 (oppmx X) i 0 != 0)).
          {by  apply /cV0Pn. }
          apply /cV0Pn. destruct H9 as [k H9]. exists k. rewrite mxE.
...
{ split. 
  ...
  + intros.
    ...
    (*specialize (H3 i). *)  apply right_eigen_vector_exists in H8.
    destruct H8 as [v H8]. 
    ...
    {  apply Rinv_r. apply non_zero_vec_norm. destruct H8.
       unfold vec_not_zero.
       assert (exists i, v i 0 != 0).
    ...
    { unfold S_mat. apply eigen_power. fold S_mat.
      rewrite -scal_vec_mathcomp_compat_col. apply H8. 
    }
    ...
    { nra. } rewrite H11.
    apply is_lim_seq_mult'.
    apply H7. apply is_lim_seq_const. 
    apply is_lim_seq_const. apply non_zero_vec_norm. destruct H8.
    unfold vec_not_zero.
    assert (exists i, v i 0 != 0).
  ...
  + intros.
    ...
    assert (RtoC_mat
                ((oppmx (mulmx (invmx A1) A2))^+n0.+1) =
                  (RtoC_mat (oppmx (mulmx (invmx A1) A2)))^+n0.+1).
    { apply mat_power_R_C_compat. } by rewrite H8. 
  ...
  + intros. 
    ...
    { assert (RtoC_vec (x0 - X) = 0 \/ RtoC_vec (x0 - X) != 0).
      ...
      + left.
        ...
        specialize (H8 x y). rewrite mxE in H8. 
        assert (((x0 - X) x 0 +i* 0)%C == (0: 'M[complex R]_(n.+1,1)) x y).
...
{ split. 
  ...
  + intros.
    ...
    { apply Rabs_right. apply Rle_ge. apply C_mod_ge_0. } rewrite <-H9.
    apply /RltP. apply (@is_lim_seq_geom_nec (C_mod (lambda S_mat i))).
    specialize (H3 i).  apply right_eigen_vector_exists in H3.
    destruct H3 as [v H3]. 
    apply (is_lim_seq_ext  (fun n0 : nat => ((C_mod (lambda S_mat i) ^ n0.+1)* 
              ((vec_norm_C  v) / (vec_norm_C v)))%Re)
              (fun n0 : nat => (C_mod (lambda S_mat i) ^ n0.+1)%Re)).
